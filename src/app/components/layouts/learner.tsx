...
const Sidebar = () => {
  ...
  if (isMobile) {
    ...
    return (
      <>
        {/* Mobile top nav */}
        <div className="fixed top-0 left-0 right-0 z-50 bg-blue-700 shadow-md px-4 py-3 flex items-center justify-between">
        ...
        {isSidebarOpen && (
          <div className="fixed top-0 left-0 h-full w-40 bg-blue-700 mt-4 shadow-xl flex flex-col transition-all duration-300 z-40 rounded-r-xl overflow-hidden">
            <button
            ...
            <nav className="flex flex-col gap-4">
              <SidebarItem icon={<FaHome />} label="Home" link="/learner/dashboard" isOpen />
  ...
  return (
    <div
      className={clsx(
        "h-screen bg-blue-700 shadow-md transition-all duration-300 overflow-hidden rounded-r-xl",
        isOpen ? "w-64" : "w-20"
      )}
    >
      <div className="p-4">
        ...
        >
          <Image src="/images/logo.png" alt="Logo" width={32} height={32} />
          {isOpen && <span className="text-stone-200 text-xl font-bold">TRA Learner</span>}
        </button>

        <nav className="flex flex-col gap-6 mt-4">
          <SidebarItem icon={<FaHome />} label="Home" link="/learner/dashboard" isOpen={isOpen} />
